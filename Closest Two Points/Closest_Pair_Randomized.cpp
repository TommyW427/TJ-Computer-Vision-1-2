...
//Approach 4: Randomized Shuffle with grid
...
vector<point> shuffle(vector<point> &vec) {
    int s = vec.size()-1;
    int i = 0;
    
    mt19937 shuffler(time(NULL));
    uniform_int_distribution<int> range(0, s);
        
    for (vector<point>::iterator iter = vec.begin(); iter!=vec.end(); ++iter) {
        int randomIndex = range(shuffler);
        iter_swap(iter,vec.begin()+randomIndex);
        range.param(uniform_int_distribution<int>::param_type(i,s));
        i++;
    }
...
struct hash_pair {
    ...
    size_t operator()(const pair<class1, class2>& coordPair) const {
        auto h1 = hash<class1>{}(coordPair.first);
        auto h2 = hash<class2>{}(coordPair.second);
        return h1 + h2;
    }
};

unordered_map<pair<unsigned long long int, unsigned long long int>, point, hash_pair> makeDictionary(double delta, vector<point> &pts, int endIndex) {
    double recDelta = 1/(delta*0.5);
    unordered_map<pair<unsigned long long int, unsigned long long int>, point, hash_pair> newDic = {};
    for (int i=0; i<=endIndex; i++) {
        point pi = pts.at(i);
        newDic.insert(pair<pair<unsigned long long int, unsigned long long int>,point>({floor(pi.getX()*recDelta),floor(pi.getY()*recDelta)},pi));
    }
    return newDic;
}
...
twoPoint part4() {
    ...
    ptsVector = shuffle(ptsVector);

    point zero = ptsVector.at(0);
    point one  = ptsVector.at(1);
    double delta = zero.distanceTo(one);
    double recDelta = 1/(0.5*delta);
    double minD = delta;
    unordered_map<pair<unsigned long long int, unsigned long long int>, point, hash_pair> grid = {
        {{floor(zero.getX()*recDelta),floor(zero.getY()*recDelta)},zero},
        {{floor(one.getX()*recDelta),floor(one.getY()*recDelta)},one}
    };
    double tempDist = 0.0;
    unsigned long long int xGrid; unsigned long long int yGrid;
    unsigned long long int minx; unsigned long long int miny;
    twoPoint mintP = twoPoint(zero,one);
    
    for (int i=2; i<ptsVector.size(); i++) {
        point pi = ptsVector.at(i);
        
        recDelta = 1/(0.5*delta);
        xGrid = floor(pi.getX()*recDelta);
        yGrid = floor(pi.getY()*recDelta);
        
        for (unsigned long long int x = max(xGrid,(unsigned long long int) 2)-2; x <= xGrid+2; x++) {
            for (unsigned long long int y = max(yGrid,(unsigned long long int) 2)-2; y <= yGrid+2; y++) {
                if (grid.find({x,y}) != grid.end()) {
                    tempDist = pi.distanceTo(grid.at({x,y}));
                    if (tempDist<minD) {
                        minD = tempDist;
                        mintP = twoPoint(pi,grid.at({x,y}));
                    }
                }
            }
        }
        ...
        if (minD < delta) {
            delta = minD;
            grid = makeDictionary(delta, ptsVector, i);
        }
        else {
            grid.insert(pair<pair<unsigned long long int, unsigned long long int>,point>({xGrid,yGrid},pi));
        }
...
int main() {
    ...
    twoPoint p3 = part3();
    double dp3 = p3.getDist();
    twoPoint p4 = part4();
    double dp4 = p4.getDist();
    string p13 = (p3.gp1()).toString(); string p23 = (p3.gp2()).toString();
    string p14 = (p4.gp1()).toString(); string p24 = (p4.gp2()).toString();
    ...
    cout << "INCOORECT: " << incorrect << "\n";*/
    
    fstream results; 
    results.open("results.txt",ios::out);
     
    results << "PART 3 (FULLY RECURSIVE) \n";
    ...
    results << "PART 3 (RANDOM) \n";
    results << "Closest Points: " << p14 << " " << p24 << "\n";
    results << setprecision(17) << "Smallest Distance: " << dp4 << "\n";
    results << setprecision(17) << "Time taken for part 4: " << t4.count() << "\n";
    
    cout << "PART 3 (FULLY RECURSIVE) \n";
    cout << "Closest Points: " << p13 << " " << p23 << "\n";
    cout << setprecision(17) << "Smallest Distance: " << dp3 << "\n";
    cout << setprecision(17) << "Time taken for part 3: " << t3.count() << "\n";
    cout << "PART 4 (RANDOM) \n";
    cout << "Closest Points: " << p14 << " " << p24 << "\n";
    cout << setprecision(17) << "Smallest Distance: " << dp4 << "\n";
    cout << setprecision(17) << "Time taken for part 4: " << t4.count() << "\n";
