...
//Name: Tommy Williams 
//Task: Create a convex hull using Grahams algorithm (Graham Scan)
...
point p0;
bool angleSort(point pNa, point pNb) {
    //return (p0.angleTo(pNa) < p0.angleTo(pNb));
    return (p0.getSlopeTo(pNa) < p0.getSlopeTo(pNb));
}
...
void part2() {
    ...
    while (getline(pointReader,ptsString)) {
        ...
        circle(2.0/400,x,y,"white");
    }
    
    double minY = 1.0;
    int minIndex = 0; 
    for (int i=0; i<ptsVector.size(); i++) {
        point temp = ptsVector.at(i);
        if (temp.getY()<minY) {
            minY = temp.getY();
            p0 = temp;
            minIndex = i;
    ...
    ptsVector.erase(ptsVector.begin()+minIndex);
    sort(ptsVector.begin(),ptsVector.end(),angleSort);
    
    
    stack<point> pointPile;
    pointPile.push(p0); pointPile.push(ptsVector.at(0));
    point pT;
    point prev = p0;
    point current;
    for (int i=1; i<ptsVector.size(); i++) {
        current = ptsVector.at(i);
        while (pointPile.size()>=2) {
            pT = pointPile.top();
            pointPile.pop();
            prev = pointPile.top();
            if (ttrol(prev,pT,current)) {
                ;
            }
            else {
                pointPile.push(pT);
        ...
        prev = pointPile.top();
        pointPile.push(current);
    }
    
    point first = pointPile.top();
    point prev2 = pointPile.top();
    pointPile.pop();
    while (!pointPile.empty()) {
        point t = pointPile.top();
        pointPile.pop();
        Bresenham(prev2.getX(),prev2.getY(),t.getX(),t.getY(),"b");
        prev2 = t;
    }
    Bresenham(first.getX(),first.getY(),p0.getX(),p0.getY(),"b");
    
    fstream imgFile;
    imgFile.open("grahamscan.ppm",ios::out);
    imgFile << "P3 " << xlen << " " << ylen << " 120 \n";
    for (int ay=0; ay<ylen; ay++) {
        for (int ax=0; ax<xlen; ax++) {
            imgFile << pixels[ay][ax][0] << " " << pixels[ay][ax][1] << " " << pixels[ay][ax][2] << " ";
