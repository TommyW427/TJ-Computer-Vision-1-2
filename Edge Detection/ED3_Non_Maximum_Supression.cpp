...
void part1() {
    ...
    Image gray = Image(xlen,ylen,"greyimagem.ppm",120);
    ...
    Image edgeThingy = Image(xlen,ylen,"imagem.ppm",120);
    int threshold = 18000;
...
void part2() {
    ...
    int threshold1 = 30000;
    int threshold2 = 80000;
    vector<pair<int,int>> ooga;
    
    for (int y=0; y<ylen; y++) {
        for (int x=0; x<xlen; x++) {
            if (gradient[y][x] >= threshold2) {
                edgeThingy.setAll(y,x,2);
                ooga.push_back(make_pair(y,x));
            }
            else if (gradient[y][x] >= threshold1) {
...
void part3(string input, int thres1, int thres2) {
    ...
    for (int y=1; y<ylen-1; y++) {
        ...
        for (int x=1; x<xlen-1; x++) {
            ...
            double mod45 = degAngle%45;
            if (mod45 > 22.5) {
                angular.at(y).push_back(degAngle+(45-mod45));
            }
            else {
                angular.at(y).push_back(degAngle-mod45);
            }
            //gradient.setAll(y,x,pow(sobelX(y,x,gray),2) + pow(sobelY(y,x,gray),2));
        }
        gradient.at(y).push_back(0);
    ...
    for (int y=1; y<ylen-1; y++) {
        for (int x=1; x<xlen-1; x++) {
            if (angular.at(y).at(x) == 45 || angular.at(y).at(x) == -135) {
                otherCoord1 = make_pair(y-1,x+1); otherCoord2 = make_pair(y+1,x-1);
            }
            else if (angular.at(y).at(x) == 90 || angular.at(y).at(x) == -90) {
                otherCoord1 = make_pair(y-1,x); otherCoord2 = make_pair(y+1,x);
            }
            else if (angular.at(y).at(x) == 0 || angular.at(y).at(x) >= 180 || angular.at(y).at(x) <= -180) {
                otherCoord1 = make_pair(y,x+1); otherCoord2 = make_pair(y,x-1);
            }
            else if (angular.at(y).at(x) == -45 || angular.at(y).at(x) == 135) {
                otherCoord1 = make_pair(y+1,x+1); otherCoord2 = make_pair(y-1,x-1);
            }
            else {
                cout << angular.at(y).at(x);
            }
...
int main(int argc, char** argv) {
    ...
    for (int i=0; i<argc; i++) {
        if (argv[i]=="-L" && argc>i+1) {
            threshold1 = atoi(argv[i+1]);
        }
        else if (argv[i]=="-H" && argc>i+1) {
            threshold2 = atoi(argv[i+1]);
        }
        else if (argv[i]=="-F" && argc>i+1) {
            image = string(argv[i+1]);
        }
    }
    part3(image,threshold1,threshold2);
